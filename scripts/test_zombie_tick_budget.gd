...
func _test_near_runs_every_tick(test_root: Node) -> void:
	...
	for tick in 5:
		var simulated: float = budget.consume(TICK, NEAR, 25.0, false)
	...
	var near_runs := 0
	for _tick in 6:
		if near_budget.consume(TICK, NEAR, 400.0, false) > 0.0:
			near_runs += 1
	if near_runs != 3:
...
func _test_far_accumulates_skipped_ticks(test_root: Node) -> void:
	...
	for _tick in far_stride * 2:
		results.append(budget.consume(TICK, FAR, 3600.0, false))
	...
	for _tick in mid_stride * 4:
		if mid.consume(TICK, MID, 900.0, false) > 0.0:
			mid_runs += 1
	if mid_runs != 4:
...
func _test_phase_spreads_zombies_across_ticks(test_root: Node) -> void:
	...
	for phase_seed in far_stride * 2:
		...
		for tick in far_stride:
			if budget.consume(TICK, FAR, 3600.0, false) > 0.0:
...
func _test_leap_forces_every_tick(test_root: Node) -> void:
	...
	budget.consume(TICK, FAR, 3600.0, false)
	var simulated: float = budget.consume(TICK, FAR, 3600.0, true)
