...
SURVIVAL_PORT="$port" SURVIVAL_TEST_MODE=--smoke-test-zombie docker compose --profile survival --project-directory "$project_dir" up --build --detach survival
sleep 3
set +e
...
set -e
printf '%s\n' "$bot_output"
server_logs="$(SURVIVAL_PORT="$port" docker compose --profile survival --project-directory "$project_dir" logs survival)"
printf '%s\n' "$server_logs"
