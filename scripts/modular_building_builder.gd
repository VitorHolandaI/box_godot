...
static func build_procedural_building(kind: String, rng: RandomNumberGenerator) -> StaticBody3D:
	...
	cutout_wall_mat.set_shader_parameter("cutout_radius", AURA_CUTOUT_RADIUS)
	...
	cutout_trim_mat.set_shader_parameter("cutout_radius", AURA_CUTOUT_RADIUS)
...
static func _create_floor_and_ceiling(body: StaticBody3D, w: int, d: int, floors: int) -> void:
	...
	cutout_slab_mat.set_shader_parameter("cutout_radius", AURA_CUTOUT_RADIUS)
