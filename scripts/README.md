...
- `capture_shots.sh`: captura os frames de divulgacao do README pelo modo `--capture` do jogo e otimiza para `docs/imagens/` (`--shot=NOME`, `--list`, `--no-optimize`; brutos em `dist/capturas/`). As receitas `carro-dirigindo` e `fps-combate` usam o Movie Maker do Godot e convertem o trecho de gameplay para GIF.
- `capture_multiplayer_gif.sh`: sobe TDM local com quatro bots, conecta o cliente de captura e gera `multiplayer-pvp.gif` a partir dos snapshots de jogadores remotos.
...
- `city_generator.gd`: geracao deterministica de ruas, calcadas urbanas de concreto, Safehouse com recorte por limites, veiculos oxidados/queimados e muralhas. No mata-mata a casa central NAO existe (as bases sao as duas dos times, em cantos opostos) e cada base ganha um carro proprio, na cor do time. Expoe `get_minimap_layout()` (ruas e pegadas dos predios) para o minimapa desenhar o mapa gerado de fundo.
...
- `drivable_car.gd`: carro dirigivel (`VehicleBody3D`) que o jogador ocupa com "interagir" (E); le steer/throttle/brake do motorista, mantem o boneco visivel sentado no assento (a camera passa a seguir e aproximar o carro) e atropela zumbi E jogador acima de 4 km/h pela `RunOverArea` (quem esta a bordo do proprio carro nunca e atingido). Tem vida/gasolina, dano de batida, assento do atirador em pe e HUD. `unlimited_fuel`/`indestructible` fazem o carro de base do mata-mata: tanque que nao seca e lataria que nao morre. O modelo do jipe militar CC-BY encosta no chao depois que a suspensao assenta (`test_ground_contact.gd`). No cliente de rede vira proxy: congela a fisica e interpola o snapshot do carro (`get_network_state`/`apply_network_state`); o snapshot leva as chaves `driver_key`/`gunner_key`, entao o cliente prende o proxy do jogador ao carro (`driving_car`/`riding_car`) para camera FPS, HUD e pose.
- `car_bot_driver.gd`: motorista de IA; segue uma rota circular (centrada em `route_center`) e entrega steer/throttle pelo mesmo contrato do jogador (`get_vehicle_input`). No servidor dedicado/Docker entra com `--car-bot=N` para testar a rede do carro sem humano ao volante.
...
- `game_config.gd`: controles, preferencias graficas (resolucao/qualidade/tela cheia) e de jogabilidade (mira pelo mouse e primeira pessoa) e servidores favoritos persistentes. `set_menu_open(tree, aberto)` centraliza o par menu aberto + cursor: solta o mouse ao abrir e devolve para a camera de cada jogador local ao fechar.
...
- `load_test_options.gd`: opcoes `--prespawn-zombies=N` (teste de carga), `--pvp-bots=N` (bots de mata-mata no servidor, para jogar player vs bot sem outro cliente) e `--car-bot=N` (bot motorista nos carros dirigiveis do servidor, para testar a rede do veiculo) do servidor dedicado; `--smoke-test-swat` (main.gd) chama o esquadrao perto do primeiro jogador, spawna alvos em anel e imprime o estado dos 4 soldados a cada 2 s ate o esquadrao expirar.
...
- `modular_building_builder.gd`: gerador de predios procedurais com andares multiplos andaveis, escadas reais transitaveis, sacadas, terraco caminhavel, iluminacao e materiais de dois tons. Cada andar vira um `FloorBuild` e e montado em tres paredes (frente, fundo e laterais) na ordem fixa que o RNG da cidade espera.
...
- `swat_squad_director.gd`: conduz o esquadrao SWAT chamado pelo jogador: cria os 4 soldados em formacao, alimenta a IA deles, solta quando o tempo acaba ou o dono some. Saiu do `main.gd`, que ficou so com os RPCs.
- `swat_smoke_test.gd`: harness de dev do `--smoke-test-swat`: no servidor cerca o jogador de zumbis, chama o esquadrao e reporta os soldados a cada 2 s; no cliente confere que eles existem ali e que se movem (o bug do id de peer negativo deixava o soldado parado na origem, invisivel e sem erro no log).
...
- `test_weapon_anchor.gd`: regressoes da ancora da arma: na postura de tiro a arma fica na ponta do braco direito (a mao), nao num offset fixo.
...
- `test_zombie_hit_zones.gd`: regressoes do dano localizado: zona pela parte mais proxima do impacto, cabeca dobrando o dano, perna derrubando a velocidade, braco enfraquecendo o golpe, duas pernas virando rastejo baixo e dano sem ponto de impacto (melee/fogo) como torso.
...
- `player_animator.gd`: gerenciador procedural de poses, marcha, animacao expressiva de impacto/flinch e a pose cosmetica de recarga (mao esquerda desce e o cano baixa; le `reload_anim_time` do jogador). Na mira a arma e ancorada na ponta do braco direito (`HAND_ANCHOR_LOCAL`), com o coice no offset.
...
- `pvp_server_director.gd`: mata-mata do lado que simula: times e cor do uniforme, cria e conduz os bots (rota pelo anel de ruas entre as duas bases), respawn continuo com invulnerabilidade, escolha de arma e o estado do HUD que vai para os clientes. Saiu do `main.gd`, que ficou so com os RPCs e o estado replicado.
- `tdm_match.gd`: regras do mata-mata por times (Team Deathmatch) autoritativo: 2 times equilibrados, placar continuo de abates ate 50 (teto de 10 min), respawn em 5 s com 4 s de invulnerabilidade (o primeiro tiro cancela), fogo amigo bloqueado e punido no placar, e as duas cores de time. Sem rodadas e sem economia — substituiu o formato estilo CS, que deixava a sala parada esperando rodada. A arma e escolhida de graca, mas a DURABILIDADE continua valendo e quem morre larga a arma no chao com o desgaste que ela tinha.
...
- `loadout_menu.gd`: menu de arma do mata-mata (tecla B) com TODO o arsenal de crate de graca, o time do jogador, o placar e a resposta do servidor; a escolha vai por RPC e vale na hora dentro da propria base, ou no proximo respawn fora dela. Abrir/fechar passa por `set_panel_open`, que solta o cursor junto (em primeira pessoa o mouse fica preso e sem isso nao dava para clicar na arma).
...
- `test_multiplayer_peers.sh`: teste ponta-a-ponta de N peers reais (padrao 8, decisao do dono): 1 servidor dedicado + N clientes bot em processos separados, exigindo N handshakes aceitos, `BOT_TEST_PASS` em todos e zero erro de RPC. Entradas escalonadas e uma retentativa por bot (o bot de teste anda em linha reta e pode enroscar numa parede do abrigo). Uso: `scripts/test_multiplayer_peers.sh [N] [porta]`.
- `test_pvp_peers.sh`: teste ponta-a-ponta de N peers reais em PVP (padrao 8): servidor `--pvp` com bots de servidor (`--pvp-bots`) para o combate + N clientes bot, exigindo N handshakes, `pvp_round`, `pvp_kill` e `BOT_TEST_PASS` em todos. Uso: `scripts/test_pvp_peers.sh [N] [porta]`.
- `test_session.sh`: checagem padrao de sessao em tres fases: (1) servidor + bot exigindo `BOT_TEST_PASS` (conectou, andou, gastou stamina, viu bala e matou zumbi), (2) servidor com `--smoke-test-swat` + cliente conferindo que os 4 soldados existem NO CLIENTE e se movem (`swat_seen_done`) e (3) cliente com `--fake-build=` recusado no handshake sem entrar no roster nem gerar `Unable to send packet`, alem da linha de build nos dois lados, handshake aceito e zero erro de RPC.
...
- `safehouse_door_sync.gd`: estado de TODAS as portas de safehouse num bitmask do snapshot de jogador (um bit por porta, ordem pelo caminho do no). Antes so a porta da casa central viajava num `bool`, e as duas portas de base do mata-mata ficavam fechadas para sempre no cliente — que nunca decide sozinho.
- `solid_venue.gd`: construcao oca (piso, paredes e porta) e colisao de casas, lojas e apartamentos.
- `split_screen_manager.gd`: viewports com audio 3D, cameras (isometrica ou primeira pessoa por jogador), HUDs locais, retículo de mira (`aim_reticle.gd`) e minimapa com o mapa gerado de fundo (ruas e predios), a seta de direcao de cada boneco, os aliados (no mata-mata SO os do proprio time, na cor do time), os zumbis dentro do raio do sonar e todos os zumbis quando restam 5 ou menos.
- `test_combat_and_variants.gd`: testes de combate, bloqueio por paredes, trajetoria, variantes, contato do corpo com o chao, vidas, safehouse, som, hordas e populacao global.
...
- `test_ground_contact.gd`: regressao de pose/capsula do zumbi rastejante e altura do jipe com a suspensao assentada.
...
- `test_tdm_match.gd`: regressoes do mata-mata por times (times equilibrados, abate pontuando o time, fogo amigo punido e bloqueado, fim por limite de abates e por tempo, empate, invulnerabilidade de respawn, as duas cores de time e o arsenal livre do loadout).
...
- `server_menu.sh`: menu do servidor local no docker compose — escolhe no ato qual MODO subir (`survival`, `tdm`, `classic`), cada um seu container, TODOS na mesma porta aberta da VPS (subir um derruba o outro), com ou sem rebuild, e ainda parar/logs/status por modo ou geral. Aceita modo+acao por argumento (`tdm up`, `survival start`), variaveis `PVP_BOTS`/`CAR_BOT`/`PORT`/`SERVER_NAME`, `DRY_RUN=1` para so mostrar o comando e `self-test` (checa o script sem docker).
...
- `zombie_limb_debris.gd`: membro arrancado em combate, visual balistico leve e temporario sem corpo rigido na horda.
- `zombie_limb_state.gd`: bits compartilhados de membros perdidos entre zumbi, snapshot e ragdoll.
...
- `zombie_snapshot_codec.gd`: snapshot binario de zumbis (17 bytes vivo, 23 morto): tipo, morte, sequencia de ataque e mascara de membros perdidos em pacotes abaixo do MTU.
