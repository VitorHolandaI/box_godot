...
- `loadout_menu.gd`: menu de arma do mata-mata (tecla B) com TODO o arsenal de crate de graca, o time do jogador, o placar e a resposta do servidor; a escolha vai por RPC e vale na hora dentro da propria base, ou no proximo respawn fora dela.
...
- `server_menu.sh`: menu do servidor local no docker compose — escolhe no ato qual MODO subir (`survival`, `tdm`, `classic`), cada um seu container e sua porta, com ou sem rebuild, e ainda parar/logs/status por modo ou geral. Aceita modo+acao por argumento (`tdm up`, `survival start`), variaveis `PVP_BOTS`/`CAR_BOT`/`PORT`/`SERVER_NAME`, `DRY_RUN=1` para so mostrar o comando e `self-test` (checa o script sem docker).
