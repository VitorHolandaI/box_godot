...
- `zombie_lab.gd`: cena de teste no editor (F6): dirige o player com o PlayerBotAI e deixa cada zumbi instanciado simular sozinho; as teclas `[` `]` escolhem a variante, `-` `=` a quantidade (1 a 100), `K` spawna na frente do player, `L` mata todos, `M` limpa a cena, e `--lab-spawn=N` spawna N de largada para teste headless.
